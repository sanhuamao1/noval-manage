...
| 场景 | 关键步骤 | 详细 |
|------|----------|------|
| **新建 CRUD 页面** | 1. `crud-config.ts` 注册实体 → 2. 复制 [页面模板](page-combine.md) → 3. 改实体 key | [page-combine.md](page-combine.md) |
| **修改 YML 配置** | 1. 编辑 `configs/*.yml` → 2. 跑 `node scripts/generate-configs.cjs` | [yml-guide.md](yml-guide.md) |
| **AI 工作流页面** | `useFactory` + Tab 缓存，见模板 | [factory-page.md](factory-page.md) |
| **加新实体** | `crud-config.ts` + 新建 `.yml` + `generate-configs.cjs` entities 数组 | [yml-guide.md](yml-guide.md) |
...
| **发 API 请求** | `api()` from `@/lib/api` | [api-guide.md](api-guide.md) |
| **组装 AI Prompt** | `callAIChat()` / `callAI()` / prompt 模板 | [ai-guide.md](ai-guide.md) |
