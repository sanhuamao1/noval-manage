...
import { renderSections, resolveIcon } from "@/lib/configs/render-utils";
...
export default function PolishPage() {
  ...
  const { fields: ruleFields, sections: ruleSections, defaults: ruleDefaults } = getEntry(ConfigEntity.POLISH_RULE);
  const { fields: sampleFields, sections: sampleSections, defaults: sampleDefaults } = getEntry(
    ConfigEntity.POLISH_SAMPLE,
  );
  ...
  function openForEdit(type: "rule" | "sample", item: PolishRule | PolishSample) {
    ...
    setEditorConfig(
      fillConfig(
        item,
        type === "rule" ? ruleDefaults : sampleDefaults,
        type === "rule" ? ruleFields : sampleFields,
      ),
    );
  }
  ...
  const editingRule =
    editingId && editingType === "rule" ? (rules.find((r) => r.id === editingId) ?? null) : null;

  const editingSample =
    editingId && editingType === "sample"
    ...
    <PageLayout
      drawer={
        <SlidingDrawer
          ...
          onCreate={
            mode === "create" ? (editingType === "rule" ? saveRule : saveSample) : undefined
          }
          ...
          {samples.map((sample) => {
            ...
            return (
              ...
              >
                    ...
                    (() => {
                      ...
                      return (
                        <Tag
                          ...
                          color={(opt?.color as OptionColor) ?? "default"}
                        >
                          {sceneType}
                        </Tag>
                      );
                ...
                {cfg.text && (
                  <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{cfg.text}</p>
                )}
              </SimpleCard>
            );
          })}
