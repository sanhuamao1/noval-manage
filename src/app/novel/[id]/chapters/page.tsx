...
export default function ChaptersPage() {
  ...
  const saveChapter = useCallback(async () => {
    ...
    await fetch("/api/chapters", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: selectedChapter.id, title: editTitle, content: editContent }),
    })
    setSaving(false)
    fetchChapters()
        ...
        <div className="w-72 border-r p-4 overflow-auto flex-shrink-0">
          ...
          <div className="flex gap-1 mb-3 bg-muted rounded-lg p-1">
            {filterTabs.map((tab) => (
              <button
                ...
                className={`flex-1 text-xs py-1.5 rounded-md transition-colors ${filter === tab.key
          ...
          <div className="space-y-1">
            {filteredChapters.map((chapter, index) => (
              <div
                key={chapter.id}
                className={`group flex items-center gap-2 px-3 py-2 rounded-md cursor-pointer text-sm transition-colors ${selectedChapter?.id === chapter.id
              ...
              >
                <span
                  className={`w-2 h-2 rounded-full flex-shrink-0 ${chapter.status === "published" ? "bg-green-500" : "bg-amber-500"
                    }`}
                />
          ...
          {selectedChapter ? (
            <>
              ...
              <div className="flex-1 p-4 relative">
                <ChapterEditor editContent={editContent} setEditContent={setEditContent} editorRef={editorRef} />
                <SelectionMenu />
                <PolishResultPopover />
              </div>
            </>
